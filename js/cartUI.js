 ...
 *
 * initCartUI() wires up the DOM once and returns a small controller with
 * updateMenu(), so main.js can swap in the live sheet data later without
 * re-attaching click listeners a second time (which would double-fire them).
 * ---------------------------------------------------------------------------
 */
...
function buildItemLookup(menu) {
  return new Map(menu.flatMap((category) => category.items).map((item) => [item.id, item]));
}
...
/**
 * Wires up the cart bar/drawer DOM once and starts listening for cart changes.
 * Returns { updateMenu(menu) } so the caller can point it at fresh menu data
 * (e.g. once the live Google Sheet loads) without re-binding click handlers.
 */
export function initCartUI(menu) {
  let itemLookup = buildItemLookup(menu);
  ...
  return {
    updateMenu(newMenu) {
      itemLookup = buildItemLookup(newMenu);
      render(elements, itemLookup); // refresh immediately in case the cart already has items
    }
  };
}/**
 * cartUI.js
